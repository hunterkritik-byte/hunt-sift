...
def local_path(value: str) -> Path:
    ...
    if not path.exists(): raise argparse.ArgumentTypeError(f"Local input does not exist: {path}")
    return path
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for name, help_text in (("nmap","Analyze a previously exported Nmap XML file."),("burp","Analyze a previously exported Burp Suite XML file."),("har","Analyze a previously saved HTTP Archive (HAR) file."),("http","Analyze a previously saved raw HTTP response."),("request","Analyze a previously saved raw HTTP request."),("s3","Analyze a previously exported S3-style bucket policy JSON file."),("static","Review local source with non-executing pattern checks."),("source","Analyze source or JavaScript with non-executing security rules."),("jwt","Decode and review a locally supplied JWT without verifying or transmitting it."),("graphql","Review a saved GraphQL request/query for security configuration cues."),("openapi","Review a saved OpenAPI/Swagger JSON specification."),("secrets","Scan local text for secret-like patterns with mandatory redaction.")):
        command = commands.add_parser(name, help=help_text); command.add_argument("--input", required=True, type=local_path)
        if name == "http": command.add_argument("--url", help="Optional original URL used only as a local label; it is never requested.")
    templates = commands.add_parser("test-templates", help="Generate inert manual-review templates from a saved HTTP request."); templates.add_argument("--input", required=True, type=local_path)
    inventory = commands.add_parser("inventory", help="Build a local project index with SHA-256 fingerprints."); inventory.add_argument("--input", required=True, type=local_path); inventory.add_argument("--output")
    search = commands.add_parser("search", help="Search a local inventory JSON file."); search.add_argument("--input", required=True, type=local_path); search.add_argument("--query", required=True)
    report = commands.add_parser("report", help="Convert a saved finding list into JSON or SARIF."); report.add_argument("--input", required=True, type=local_path); report.add_argument("--output", required=True); report.add_argument("--format", choices=("json","sarif"), default="json")
    diff = commands.add_parser("diff", help="Compare two saved HTTP responses without sending requests."); diff.add_argument("--before", required=True, type=local_path); diff.add_argument("--after", required=True, type=local_path)
    endpoints = commands.add_parser("endpoints", help="Build a deduplicated endpoint map from a local artifact."); endpoints.add_argument("--input", required=True, type=local_path)
    params = commands.add_parser("params", help="Mine and classify parameter names from a local artifact."); params.add_argument("--input", required=True, type=local_path)
...
def load_leads(path: Path) -> list[Lead]:
    ...
    if not isinstance(payload, list): raise ValueError("Finding JSON must contain a list")
    leads=[]
    for row in payload:
        if not isinstance(row, dict): continue
        required=("source","category","severity","message","evidence","guidance")
        if not all(k in row for k in required): raise ValueError("Each finding must contain source, category, severity, message, evidence, and guidance")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "inventory":
            records=index_directory(args.input); write_index(records,args.output) if args.output else None; print(json.dumps([r.__dict__ for r in records],indent=2) if args.json else f"Indexed {len(records)} local artifacts."); return 0
        if args.command == "search":
            data=json.loads(args.input.read_text(encoding="utf-8")); needle=args.query.casefold(); rows=[r for r in data if needle in str(r.get("path","")).casefold() or needle in str(r.get("kind","")).casefold()]; print(json.dumps(rows,indent=2) if args.json else "\n".join(f"{r.get('kind','other'):12} {r.get('path','')}" for r in rows) or "No matching artifacts."); return 0
        if args.command == "report":
            leads=load_leads(args.input); (write_sarif if args.format=="sarif" else write_json)(args.output,leads); print(f"Wrote {args.format} report with {len(leads)} local review leads to {args.output}."); return 0
    ...
    except (ValueError,OSError,json.JSONDecodeError) as error: print(f"Input error: {error}"); return 2
